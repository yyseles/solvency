...
BASE = r"C:/Users/yangyang05-ghq/Desktop/wb"
SOLV_DIR = os.path.join(BASE, "solvency")
...
def is_q4_conflict(pk, pdata):
    """pk 是某年Q4，且同年存在年度数据 → 冲突，应剔除"""
    m = re.fullmatch(r'(\d{4})Q4', pk)
    if not m: return False
    y = m.group(1)
    return any(re.fullmatch(r'\d{4}', k) and k == y for k in pdata)

def drop_conflict_q4(D):
    """剔除所有『同年有年度时的Q4』；无年度的Q4 保留。返回剔除条数。"""
    cnt = 0
    for seg, obj in D["segments"].items():
        for comp, pdata in obj["data"].items():
            for pk in list(pdata.keys()):
                if is_q4_conflict(pk, pdata):
                    del pdata[pk]
                    cnt += 1
    return cnt
...
# 剔除同年有年度时的Q4（保留无年度的Q4）
dropped = drop_conflict_q4(D)
if dropped:
    print(f"剔除冲突Q4: {dropped} 条")
