...
(function(){
  ...
  function renderAllOverview(){
    ...
    [['group','allBar_group'],['property','allBar_property'],['life','allBar_life'],['reins','allBar_reins']].forEach(([seg,id])=>{
      ...
      setOpt(id,{
        ...
        grid:{left:55,right:18,top:50,bottom:52},
        xAxis:{type:'category',data:['综合偿付能力充足率','核心偿付能力充足率'],
          axisLabel:{interval:0, formatter:v=> v.replace('偿付能力充足率','\n偿付能力充足率') , lineHeight:14, margin:10}},
  ...
  function renderEvoChart(id,field){
    ...
    setOpt(id,{
      ...
      grid:{left:50,right:52,top:52,bottom:62},
  ...
  function renderDist(){
    ...
    setOpt('distStack',{
      ...
      grid:{left:45,right:20,top:35,bottom:62},
      xAxis:{type:'category',data:labels,axisLabel:{rotate:35,interval:0}},
  ...
  function renderEquityPieCmp(k, targetId){
    ...
    setOpt(targetId||'equityPieCmp',{
      tooltip:{trigger:'axis',axisPointer:{type:'shadow'},valueFormatter:v=>v==null?'—':v.toFixed(2)+'%'},
      legend:{data:['公司占比','行业占比'],top:0},
      grid:{left:88,right:30,top:35,bottom:30},
      xAxis:{type:'value',name:'占比%',axisLabel:{formatter:v=>v.toFixed(0)+'%'}},
      yAxis:{type:'category',data:cats,axisLabel:{fontSize:11,interval:0,
        formatter:v=> [...v].length>8 ? v.replace(/(.{8})/g,'$1\n') : v, lineHeight:13}},
      series:[
  ...
  function renderNetAssetRatio(){
    ...
    setOpt('netAssetRatioChart',{
      ...
      grid:{left:60,right:60,top:48,bottom:58},
      xAxis:{type:'category',data:labels,axisLabel:{rotate:35,fontSize:10,interval:0}},
  ...
  function renderSecTrend(sec){
    ...
    zhMetrics.forEach((metric, mi)=>{
      ...
      const option = {
        ...
        grid:{ left:58, right:22, top:40, bottom:56 },
        xAxis:{ type:'category', data:displayPeriods, axisLabel:{ fontSize:11, interval:0, rotate: displayPeriods.length>8?30:0 }, boundaryGap:false },
  ...
  function renderSecCompare(sec){
    ...
    zhMetrics.forEach((metric, mi)=>{
      ...
      // 横轴长名（如"上市集团加权平均"）旋转会溢出截断 → 改为语义化换行、不旋转
      // 断行策略：优先把 "(口径)" 尾部分离并独立成行，主名部分每6字断行，
      // 避免 "XXX加权平均(监管口径)" 括号尾巴残行、碎片单字行等难看排版。
      const lineOf = (v)=>{ const a=[...v]; if(a.length<=6) return [v];
        const out=[]; for(let i=0;i<a.length;i+=6) out.push(a.slice(i,i+6).join('')); return out; };
      const splitPair = (n)=>{
        const m = String(n).match(/^(.*?)(\([^)]*\))?$/);
        const head = m?(m[1]||''):'', tail = (m&&m[2])||'';
        let lines = head ? lineOf(head) : [];
        if(tail) lines = lines.concat(lineOf(tail));
        return lines.length?lines:[String(n)];
      };
      const lineCountOf = n=> splitPair(n).length;
      const wrapAxis = v=> splitPair(v).join('\n');
      // 横轴所需底部空间：按最长实体的实际换行行数动态计算（与 wrapAxis 同一算法，保证高度一致），避免长名显示不全
      const maxLines = names.reduce((m,n)=> Math.max(m, lineCountOf(n)), 1);
      const needBottom = Math.max(34, maxLines*14 + 6); // 每行14px(含行距) + 6 边距
      const cBottom = many ? Math.max(60, needBottom) : needBottom;
      ...
      const option = {
        ...
        grid:{ left:58, right:50, top:36, bottom: cBottom },
  ...
  function renderSecTable(sec){
    ...
    // 期次过滤：group 板块只看 Q2/Q4，倒序最新在前
    const dispOf = arr => (sec==='group')
    ...
    zhMetrics.forEach(metric=>{
      ...
      S.entities.forEach(ent=>{
        ...
        const dispArr = dispOf(entityArr(ent, metric));
      ...
      filteredCompanies.forEach(item=>{
        ...
        const dispVals = dispOf(periods.map(p=> rVals[p]));
    ...
    // 段3：资本明细（实际/核心/附属/最低 各一组，组内实体+公司）
    h += '<tr class="blk"><td colspan="'+(revPeriods.length+1)+'">资本明细(亿元)</td></tr>';
    CAP_METRICS.forEach(cmName=>{
      h += '<tr class="sub subhd"><td style="padding-left:12px">└ '+cmName+'</td></tr>';
      S.entities.forEach(ent=>{
        if(ent.src==='reg') return;   // 监管披露口径无资本数据
        const dispCArr = dispOf(capArr(ent, cmName));
        h += '<tr class="sub agg"><td style="padding-left:22px">'+ent.name+'</td>';
      ...
      filteredCompanies.forEach(item=>{
        ...
        const dispCVals = dispOf(periods.map(p=> cVals[p]));
        h += '<tr class="sub co'+(cls?' '+cls:'')+'"><td style="padding-left:22px">'+c+'</td>';
